...
- **触发方式**：`@机器人` 或消息里带「薇欧拉 / 薇欧拉酱 / viola」
- **群开关**：默认关闭，管理员发 `/ai on` 开启、`/ai off` 关闭
- **冷却**：连续对话间隔 5 秒
- **记忆**：按 群+用户 各自记忆最近 20 轮；`/clear` 清自己记忆，`/clear all`（超管）清全群

### 🎴 角色卡

- 每张卡 = 名称 + 简介 + 人设（system prompt），可在管理面板增删改
- **默认角色**：面板「⭐ 设默认」；**超管专属**：面板「👑 超管专属角色卡」下拉（超管 @ 时用另一张卡）
- 群里 `/role` 查看当前角色，`/role <名字>`（超管）切换

### 🔍 联网模式

- DeepSeek API 本身无联网，本项目用 **Bing 搜索增强**：面板开启「联网搜索」后，对话前自动搜索实时信息注入，能回答天气/新闻/指数等实时问题；当前时间也会注入，日期问题不会答错

### 🌐 管理面板

- 地址：管理台域名（登录后）左侧菜单：🤖 AI 管理 / 🎬 B站登录 / 🐱 NapCat 面板
- **AI 管理**：角色卡管理、群开关（每群可单独开启 + 清除记忆）、超级管理员、Token 用量图表、DeepSeek 调试、联网开关
- **B站登录**：扫码续 cookie，无需 SSH

### 🎬 B站视频

- 群里发 B站链接 / b23.tv 短链 / BV 号 → 下载视频（默认 1080P）→ **合并转发**成一条聊天记录（台词 + 信息 + 视频）发出，发送后自动删除服务器文件
- 下载或合并失败时，薇欧拉会发失败提示（不降级分开发）
- 同一会话 60 秒内只处理一次（防刷）
...
/chat <内容>     聊天
/ai on|off       本群开关（超管）
/role [名字]     查看/切换角色（切换需超管）
/clear [all]     清对话记忆（all 需超管）
```
